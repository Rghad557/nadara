...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // الحل: التأكد من وجود المفتاح أولاً وتخزينه في المتغير الذي تستخدمينه في الاستعلام $p_category
    $p_category = isset($_POST['category']) ? mysqli_real_escape_string($conn, $_POST['category']) : '';
...
if (!empty($p_id)) {
        // تحديث (Update)
        $query = "UPDATE products SET name='$p_name', category='$p_category', price='$p_price', stock='$p_stock', description='$p_desc', image='$image_name' WHERE product_id=$p_id";
        $msg = "Product updated successfully!";
    } else {
        // إضافة جديد (Insert)
        $query = "INSERT INTO products (name, category, price, stock, description, image) 
                  VALUES ('$p_name', '$p_category', '$p_price', '$p_stock', '$p_desc', '$image_name')";
